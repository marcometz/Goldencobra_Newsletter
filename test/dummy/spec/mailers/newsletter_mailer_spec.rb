  ...
  describe "Newsletter" do
    ...
    include Capybara::DSL
      ...
      it "should unsubscribe from the chosen newsletter when following 'unsubscribe' link" do
        ...
        visit "http://cloudforum.tagesspiegel.de/goldencobra_newsletter/newsletters/unsubscribe?tag=newsletter_monthly&email=holger@ikusei.de"
        my_user = User.find_by_email(user.email)
    ...
    describe "subscribing" do
      ...
      it "should send a confirmation email" do
        GoldencobraNewsletter::NewsletterMailer.should_receive(:confirm_subscription)#.with("holger@ikusei.de", "newsletter_monthly")
        visit "http://cloudforum.tagesspiegel.de/goldencobra_newsletter/newsletters/register?email=holger@ikusei.de&first_name=Holger&last_name=Frohloff&newsletter_tag=newsletter_monthly"
      end
      ...
      it "should re-subscribe a user from a link in email" do
        ...
        newsletter_template = GoldencobraEmailTemplates::EmailTemplate.create(
                                         ...
                                         content_html: "<div id='awesome-body'>My content</div>",
                                         template_tag: "newsletter_monthly")
        visit "http://cloudforum.tagesspiegel.de/goldencobra_newsletter/newsletters/subscribe?email=holger@ikusei.de&tag=newsletter_monthly"
        my_user = User.find_by_email("holger@ikusei.de")
        my_newsletter_registration = GoldencobraNewsletter::NewsletterRegistration.where('user_id = ?', my_user.id).first
        my_newsletter_registration.newsletter_tags.should == "newsletter_monthly"
      end
