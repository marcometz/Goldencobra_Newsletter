ActiveAdmin.register User, :as => "Master Data" do
  ...
  index do
    ...
    column :newsletter_registration, sortable: true do |user|
      link_to "Bearbeiten", edit_admin_newsletter_registration_path(GoldencobraNewsletter::NewsletterRegistration.where(user_id: user.id).first) if GoldencobraNewsletter::NewsletterRegistration.where(user_id: user.id).any?
    end
  end
